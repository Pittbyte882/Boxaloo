...
export async function POST(request: NextRequest) {
  ...
  if (authHeader?.startsWith("Bearer bxl_live_")) {
    ...
    try {
      ...
      console.error("API POST /loads error:", err?.message || err)
      console.error("API POST /loads details:", err?.details || "")
      console.error("API POST /loads hint:", err?.hint || "")
      return NextResponse.json({ error: "Failed to create load", detail: err?.message || "unknown" }, { status: 500 })
    }
  }
  ...
  try {
    ...
    console.error("POST /api/loads error:", err?.message || err)
    return NextResponse.json({ error: "Failed to create load", detail: err?.message || "unknown" }, { status: 500 })
  }
}
...
export async function PATCH(request: NextRequest) {
  ...
  if (authHeader?.startsWith("Bearer bxl_live_")) {
    ...
    try {
      ...
      console.error("API PATCH /loads error:", err?.message || err)
  ...
  try {
    ...
    console.error("PATCH /api/loads error:", err?.message || err)
    return NextResponse.json({ error: "Failed to update load" }, { status: 500 })
  }
}
...
export async function DELETE(request: NextRequest) {
  ...
  if (authHeader?.startsWith("Bearer bxl_live_")) {
    ...
    try {
      ...
      console.error("API DELETE /loads error:", err?.message || err)
  ...
  try {
    ...
    console.error("DELETE /api/loads error:", err?.message || err)
    return NextResponse.json({ error: "Failed to delete load" }, { status: 500 })
  }
}
